...
> [!note] These numbers describe the six-channel node
> On 2026-08-30 the node gained a dedicated min-max height channel, eight in total (master plan §2.2, reasoning in the log), and a second construction that builds each level by enumerating the texel nodes it covers instead of folding four children. Every measurement below still holds for the six Taylor channels, which are unchanged — including the min-max recovery ratios, which are what motivated the new channel. Confirmed rather than assumed: exp05 and exp06 were re-run against the eight-channel reference on 2026-08-30 and reproduce their verdicts and numbers, worst median ratio 2.77. The memory figure rises by a third. See [[Result — Pyramid channel study]] for the decomposition of the recovery gap.
