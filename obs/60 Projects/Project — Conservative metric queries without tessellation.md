...
2. **Ray query at parity** (**next**): the min-max height channel is stored (§2.2, implemented 2026-08-30), so what is left is to measure traversal — node tests and time against the pre-tessellated mesh as ground truth — for three bound choices: min-max only, slab only, both. The measured crossover says where to expect the switch: the slab is the thinner bound below 8-texel cells, the box above. Traversal cost has never been measured on this structure; only bound widths have, and the recovered interval is 5–29× too wide at the root, which is where a traversal starts. This also decides whether larger demo scenes are affordable at all, since the pre-tessellated visibility mesh does not scale (a 12.8k-triangle base at texel-matched subdivision is over 50M micro-triangles, and finer than that is where the A-MVP's MIS check needs it). If the slab wins at fine levels, that is a contribution; if it does not, the min-max channel still buys parity and the claim stays "no regression". Evidence for the channel: [[Result — Pyramid channel study]].
