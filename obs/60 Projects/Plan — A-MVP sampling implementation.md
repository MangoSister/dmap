...
- Coarse-level product weights: the midpoint √det G composition concedes 1.3–3.1× variance to the product table at amplitude 0.2 (S7). Candidate improvements: per-node sums of per-texel E·√det G (the fused bake, per-face) or certified interval midpoints. Decide whether the gap matters before investing.
- Per-node weight caching (S7 section above): implement before any equal-time claim goes in the paper.
- float32 storage with outward rounding: now has measured stakes — the pyramid is ~19× the table's memory per footprint in double, ~9× in float32 (S7 cost table).
