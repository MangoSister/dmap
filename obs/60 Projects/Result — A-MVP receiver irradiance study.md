...
- **Per footprint the table is ~27× smaller, contrary to the memory-tie expectation in §5A** (1 float per texel versus the 8 double node channels, the emission mean and max over all levels, and the triangle's boundary cache: 434 KB against 16 KB). The hierarchy's memory case needs tile reuse of roughly 27× or more, about 14× with float32 storage (open decision). This column was remeasured on 2026-09-07 after the general-chart rework (log of that date): the emission tile carries the max beside the mean, since the max is what certifies zero mass on a shared tile. It read ~24× (383 KB) with the eight-channel node and the clipped emission sums on 2026-08-30, and ~19× (298 KB) with the six-channel node. Every other number in this note reproduces bit for bit under the rework, since the identity chart's descent draws the same random stream with the same exact weights. Accounting nuance: sampling weights use only h0, gu, gv and the emission sum (~175 KB); the remainder channels (r, ru, rv) are the certificates shared with the other applications of the representation.
