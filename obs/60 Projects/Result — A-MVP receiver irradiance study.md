...
- **Per footprint the table is ~24× smaller, contrary to the memory-tie expectation in §5A** (1 float per texel versus the 8 double node channels plus the emission sum over all levels: 383 KB against 16 KB). The hierarchy's memory case needs tile reuse of roughly 24× or more, about 12× with float32 storage (open decision). This column was remeasured on 2026-08-30 under the eight-channel node; it read ~19× (298 KB) with the six-channel node, and every other number in this note is from that earlier run and is unaffected, the pyramid's construction and the six original channels being unchanged. Accounting nuance: sampling weights use only h0, gu, gv and the emission sum (~175 KB); the remainder channels (r, ru, rv) are the certificates shared with the other applications of the representation.
